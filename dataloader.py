...
class CellsDataset(Dataset):
    def __init__(self, img_root, gt_dmap_root, gt_dots_root, class_indx, split_filepath=None, train_phase='train',
                 fixed_size=-1, max_side=-1, max_scale=-1, aug=True):
    ...
    def __getitem__(self, index):
        ...
        if self.aug:
            if random.randint(0, 1) == 1 and self.train_phase == 'train':
            ...
            if self.train_phase == 'train':
                img = np.rot90(img, k=rotates, axes=(0, 1))
                gt_dmap = np.rot90(gt_dmap, k=rotates, axes=(0, 1))
                gt_dots = np.rot90(gt_dots, k=rotates, axes=(0, 1))

        # if train, make sure width and height < max_side
        if (self.train_phase == 'train' and self.max_side > 0):
            h = img.shape[0]
            w = img.shape[1]
            h2 = h
            w2 = w
        ...
        if self.train_phase == 'train':
            ...
            if (self.fixed_size < 0):
                i, j, h, w = transforms.RandomCrop.get_params(img_pil,
                                                              output_size=(img.shape[0] // 4, img.shape[1] // 4))
            elif (self.fixed_size < img.shape[0] or self.fixed_size < img.shape[1]):
                i, j, h, w = transforms.RandomCrop.get_params(img_pil, output_size=(
                    min(self.fixed_size, img.shape[0]), min(self.fixed_size, img.shape[1])))
            if (i >= 0):
                img = img[i:i + h, j:j + w, :]
        ...
        if self.max_scale > 1:  # to downsample image and density-map to match deep-model.
            ...
            img = np.pad(img, ((pad_y1, pad_y2), (pad_x1, pad_x2), (0, 0)), 'constant',
                         constant_values=(1,))  # padding constant differs by dataset based on bg color
            gt_dmap = np.pad(gt_dmap, ((pad_y1, pad_y2), (pad_x1, pad_x2), (0, 0)), 'constant',
                             constant_values=(0,))  # padding constant differs by dataset based on bg color
            gt_dots = np.pad(gt_dots, ((pad_y1, pad_y2), (pad_x1, pad_x2), (0, 0)), 'constant',
                             constant_values=(0,))  # padding constant differs by dataset based on bg color
