...
from util import CNNArchUtilsPyTorch
...
class UnetVggMultihead(nn.Module):
    def __init__(self, kwargs=None):
        ...
        args = {'conv_init': 'he', 'block_size': 3, 'pool_size': 2,
                'dropout_prob': 0, 'initial_pad': 0, 'n_classes': 1,
                'n_channels': 3, 'n_heads': 2, 'head_classes': [1, 1]}
        ...
        layer.add_module('encoder_conv_l_' + str(layer_index) + '_0',
                         nn.Conv2d(self.n_channels, 64, kernel_size=self.block_size, padding=self.pad))
        ...
        layer.add_module('encoder_conv_l_' + str(layer_index) + '_1',
                         nn.Conv2d(64, 64, kernel_size=self.block_size, padding=self.pad))
        ...
        layer.add_module('encoder_conv_l_' + str(layer_index) + '_0',
                         nn.Conv2d(64, 128, kernel_size=self.block_size, padding=self.pad))
        layer.add_module('encoder_relu_l_' + str(layer_index) + '_0', nn.ReLU(inplace=True))
        layer.add_module('encoder_conv_l_' + str(layer_index) + '_1',
                         nn.Conv2d(128, 128, kernel_size=self.block_size, padding=self.pad))
        layer.add_module('encoder_relu_l_' + str(layer_index) + '_1', nn.ReLU(inplace=True))
        self.encoder.add_module('encoder_l_' + str(layer_index), layer)
        ...
        layer.add_module('encoder_dropout_l_' + str(layer_index), nn.Dropout(p=self.dropout_prob))
        layer.add_module('encoder_conv_l_' + str(layer_index) + '_0',
                         nn.Conv2d(128, 256, kernel_size=self.block_size, padding=self.pad))
        layer.add_module('encoder_relu_l_' + str(layer_index) + '_0', nn.ReLU(inplace=True))
        layer.add_module('encoder_conv_l_' + str(layer_index) + '_1',
                         nn.Conv2d(256, 256, kernel_size=self.block_size, padding=self.pad))
        layer.add_module('encoder_relu_l_' + str(layer_index) + '_1', nn.ReLU(inplace=True))
        ...
        layer.add_module('encoder_dropout_l_' + str(layer_index), nn.Dropout(p=self.dropout_prob))
        layer.add_module('encoder_conv_l_' + str(layer_index) + '_0',
                         nn.Conv2d(256, 512, kernel_size=self.block_size, padding=self.pad))
        layer.add_module('encoder_relu_l_' + str(layer_index) + '_0', nn.ReLU(inplace=True))
        layer.add_module('encoder_conv_l_' + str(layer_index) + '_1',
                         nn.Conv2d(512, 512, kernel_size=self.block_size, padding=self.pad))
        layer.add_module('encoder_relu_l_' + str(layer_index) + '_1', nn.ReLU(inplace=True))
        layer.add_module('encoder_conv_l_' + str(layer_index) + '_2',
                         nn.Conv2d(512, 512, kernel_size=self.block_size, padding=self.pad))
        layer.add_module('encoder_relu_l_' + str(layer_index) + '_2', nn.ReLU(inplace=True))
        ...
        layer_index = 3
        layer = nn.Sequential()
        layer.add_module('decoder_deconv_l_' + str(layer_index),
                         nn.ConvTranspose2d(512, 512, stride=self.pool_size, kernel_size=self.pool_size))
        layer.add_module('decoder_conv_l_s_' + str(layer_index) + '_0',
                         nn.Conv2d(1024, 512, kernel_size=self.block_size, padding=self.pad))
        layer.add_module('decoder_relu_l_' + str(layer_index) + '_0', nn.ReLU(inplace=True))
        layer.add_module('decoder_conv_l_' + str(layer_index) + '_1',
                         nn.Conv2d(512, 512, kernel_size=self.block_size, padding=self.pad))
        ...
        layer = nn.Sequential()
        layer.add_module('decoder_deconv_l_' + str(layer_index),
                         nn.ConvTranspose2d(512, 256, stride=self.pool_size, kernel_size=self.pool_size))
        layer.add_module('decoder_conv_l_s_' + str(layer_index) + '_0',
                         nn.Conv2d(512, 256, kernel_size=self.block_size, padding=self.pad))
        layer.add_module('decoder_relu_l_' + str(layer_index) + '_0', nn.ReLU(inplace=True))
        layer.add_module('decoder_conv_l_' + str(layer_index) + '_1',
                         nn.Conv2d(256, 256, kernel_size=self.block_size, padding=self.pad))
        layer.add_module('decoder_relu_l_' + str(layer_index) + '_1', nn.ReLU(True))
        ...
        layer = nn.Sequential()
        layer.add_module('decoder_deconv_l_' + str(layer_index),
                         nn.ConvTranspose2d(256, 128, stride=self.pool_size, kernel_size=self.pool_size))
        layer.add_module('decoder_conv_l_s_' + str(layer_index) + '_0',
                         nn.Conv2d(256, 128, kernel_size=self.block_size, padding=self.pad))
        layer.add_module('decoder_relu_l_' + str(layer_index) + '_0', nn.ReLU(inplace=True))
        layer.add_module('decoder_conv_l_' + str(layer_index) + '_1',
                         nn.Conv2d(128, 128, kernel_size=self.block_size, padding=self.pad))
        layer.add_module('decoder_relu_l_' + str(layer_index) + '_1', nn.ReLU(True))
        ...
        layer = nn.Sequential()
        layer.add_module('decoder_deconv_l_' + str(layer_index),
                         nn.ConvTranspose2d(128, 96, stride=self.pool_size, kernel_size=self.pool_size))
        self.decoder.add_module('decoder_l_' + str(layer_index), layer)

        self.final_layers_lst = nn.ModuleList()
        # In Interchangeable Teacher, there are only 2 heads: cell detection, cell classification
        for i in range(self.n_heads):
            block = nn.Sequential()
            feat_subblock = nn.Sequential()
            pred_subblock = nn.Sequential()
            feat_subblock.add_module('final_block_' + str(i) + '_conv3_0',
                                     nn.Conv2d(96, 64, kernel_size=self.block_size, padding=self.pad))
            feat_subblock.add_module('final_block_' + str(i) + '_relu_0', nn.ReLU(inplace=True))
            feat_subblock.add_module('final_block_' + str(i) + '_conv3_1',
                                     nn.Conv2d(64, 64, kernel_size=self.block_size, padding=self.pad))
            feat_subblock.add_module('final_block_' + str(i) + '_relu_1', nn.ReLU(True))
            pred_subblock.add_module('final_block_' + str(i) + '_conv1_2',
                                     nn.Conv2d(64, self.head_classes[i], kernel_size=1))
